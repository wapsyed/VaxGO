...
immunego_process_total = ImmuneGO_BTM_genes %>%
  select(-genes) %>% 
  distinct() %>% 
  filter(!is.na(immune_system)) %>% 
  group_by(immune_set, immune_system, immune_subsystem, cells) %>% 
  summarize(n_processes = n()) %>% 
  filter(immune_set %in% c("BTM by Modules", "ImmuneGO by Modules")) %>% 
  arrange(process) %>% 
  mutate(immune_process_group = paste(immune_system, immune_subsystem, cells, sep = ", "))
immunego_process_total
```
...
immunego_process_total %>% 
  ungroup() %>% 
  arrange(process, immune_process_group, -n_processes) %>% 
 ggplot() +
  aes(y = process, x = n_processes, fill = immune_set) +
...
immunego_process_total %>% 
  ungroup() %>% 
  arrange(immune_process_group, immune_set, -n_processes) %>% 
 ggplot() +
  aes(y = immune_process_group %>% 
        fct_reorder(n_processes), x = n_processes, fill = immune_set) +
  geom_col(show.legend = T) +
  scale_fill_npg() +
  labs(title = "Immune set by processes",
       x = "Total processes",
       y = "",
       fill = "Immune set") +
  theme_minimal() +
...
Immune_cells_mainpro = Immune_cells %>% 
  select(marker, Type) %>%
...
```{r fig.height=5, fig.width=7}
markers_cells = Immune_cells_markers %>% 
  slice_head(n = 20) %>% 
  mutate(marker = fct_reorder(marker, n_cells)) %>% 
  ggplot() +
   aes(x = n_cells, y = marker) +
  geom_col(fill = "black") +
  labs(
    x = "Cells",
    y = "",
...
```{r fig.height=5, fig.width=7}
markers_immunesystem = Immune_cells_mainpro %>% 
  filter(marker %in% c(markers_cells$data$marker %>% as.character())) %>% 
  mutate(marker = fct_relevel(marker, markers_cells$data$marker %>% levels())) %>% 
  slice_head(n = 20) %>% 
  esquisser()
  ggplot() +
  aes(x = Type,
...
markers_immunesystem

markers_cells + markers_immunesystem +
...
```{r}
Immune_cells_mainpro %>% 
  slice_head(n = 20) %>% 
  ggplot() +
  aes(x = n_cells,
      y = marker %>% fct_reorder(n_cells),
      fill = Type) +
  geom_col() +
  scale_fill_npg() +
  theme_minimal() +
...
```{r fig.height=5, fig.width=7}
VaxSigDB_Gene_sets_Annotated_RAW <- readRDS("~/Desktop/VaxGO/Tables/VaxSigDB_Gene_sets_Annotated_RAW.rds") %>% 
  clean_names()



VaxSigDB_Gene_sets_Annotated_RAW %>% 
  select(vaccine, platform, target_pathogen_disease, microbe_type) %>% 
  distinct() %>%
  group_by(target_pathogen_disease) %>% 
  summarize(n_vaccines = n(),
            platform,
            microbe_type) %>% 
  
  ggplot() +
  aes(y = target_pathogen_disease %>% 
        fct_reorder(n_vaccines), 
      x = n_vaccines,
      fill = platform ) +
  geom_col() +
  scale_fill_npg() +
  ...
  labs(x = "Cells",
       y = "",
       fill = "Type") +
  facet_grid(vars(microbe_type %>% 
                    fct_relevel("VIRUS", "BACTERIUM", "PARASITE")), scales = "free_y",space = "free_y")+
  scale_x_continuous(expand = c(0.01, 0.01))
library(ggh4x)
...
data = CellMarker_ImmuneCells %>% 
 select(process = cell_name, genes = marker)

filename = "CellMarker_ImmuneCells"
```
...
shared_genes_df_mirror = read_csv(file = here("Tables", "CellMarker_ImmuneCells_sharedgenes_Fisher_Jaccard.csv"))
