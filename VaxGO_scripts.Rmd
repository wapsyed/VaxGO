...
example_condition = conditions_genes %>% 
  filter(condition == "BNT (V2, D1)") 

example_condition %>% 
  dplyr::select(condition:direction) %>% 
...
degs_sig_example = example_condition %>% 
  filter(padj < filter_padj,
         log2fold_change > filter_logfc | log2fold_change < -filter_logfc)
...
btm_genes = btm_annotation_table %>% 
  filter(module_category == filter_btm_category,
         annotation_level %in% filter_btm_annotationlevel) %>% 
  select(process = composite_name, genes = module_member_genes) %>% 
  separate_rows(genes, sep = ",")
...
#CellMarker immune-------
genelist_CellMarker_genes = CellMarker_genes %>% select(process, genes)
...
#BTM immune-------
genelist_btm_genes = btm_genes %>% select(process, genes)
...
#VAX MSigDB selected-------
genelist_VaxSigDB_Genes = VaxSigDB_Genes_filtered %>% select(process, genes)
