...
#### Immune genes only
df_vaccines_immune <- immune_genes_covid_shared_process %>% 
...
types_immune = df_vaccines_immune %>% 
  dplyr::select(condition = Cond1) %>% 
  distinct() %>% 
  inner_join(annotation_conditions %>% 
 dplyr::select(condition, type), by = "condition") %>% 
...
colors_grid_immune = df_vaccines_immune %>%
  rename(condition = Cond1) %>% 
 inner_join(colors, by = "condition") %>% 
 pull(color, condition)

col_links_immune = df_vaccines_immune %>% 
  inner_join(col_immune %>% 
  as.data.frame() %>% 
  rownames_to_column("process") %>% 
  rename("color" = "."), by = "process") %>% 
...
# IMMUNE
selected_links_W1_immune = df_vaccines_immune %>% 
  ungroup() %>% 
  select(Cond1, Cond2) %>% 
  inner_join(annotation_conditions %>% 
              select(Cond1 = condition, week) %>% 
...
df_vaccines_selected_W1_immune = df_vaccines_immune %>% 
  left_join(selected_links_W1_immune %>% 
              rename(Cond2 = Cond1,
                     selected_Cond2 = selected_Cond1),
             by = "Cond2") %>% 
  left_join(selected_links_W1,
...
selected_links_vector_W1_immune = df_vaccines_selected_W1_immune %>% 
  pull(selected_TF)

df_W1_immune = df_vaccines_selected_W1_immune %>% 
  select(Cond1:Shared)
...
```{r}
#Plot
pdf(file = here("Figures","ChordDiagram_Covid19_AllCovid_By_Process_All_Immune_VACCINESONLY.pdf"), 
    width = 10, 
    height = 20)
...
chordDiagram(df_vaccines, 
             ...
             directional = -1,
             scale = TRUE,
             link.zindex = rank(df_vaccines[[3]]),
             big.gap = 3,
             transparency = 0.3,
             small.gap = 0.5,
             h.ratio = 0.7,
...
circos.track(
  ...
  panel.fun = function(x, y) {
    circos.text(
      ...
      font = 0.1)},#Label face
  bg.border = NA) 
title("All - Vaccines only")


chordDiagram(df_vaccines_immune,
             grid.col = colors_grid_immune, 
             col = col_links_immune,
             link.sort = TRUE,
             link.decreasing = TRUE,
             annotationTrack = "grid", 
             directional = -1,
             scale = TRUE,
             link.zindex = rank(df_vaccines_immune[[3]]),
             big.gap = 3,
             transparency = 0.3,
             small.gap = 0.5,
             h.ratio = 0.7,
             group = group_types,
             preAllocateTracks = list(track.height = 0.5 * max(strwidth(unlist(unique(df_vaccines_immune$Cond1)))))
             )
circos.track(
  track.index = 1,
  panel.fun = function(x, y) {
...
chordDiagram(df_W1_immune, 
             ...
             small.gap = 0.5,
             h.ratio = 0.7,
             link.visible = selected_links_vector_W1_immune,
             group = group_types,
             preAllocateTracks = list(track.height = 0.5 * max(strwidth(unlist(unique(df_vaccines_immune$Cond1)))))
             )
circos.track(
