...
annotations = ann_vaccines_conditions
df = all_degs_p_05_vac_infected_19_12_23 
...
col_clustered = df %>% 
  ...
  pivot_wider(names_from = "genes",
              values_from= "log2fold_change") %>%
  select(condition, row_clustered$labels) %>% 
  column_to_rownames("condition") %>% 
  as.matrix() %>% 
  replace(is.na(.), 0) %>% 
  dist(.) %>% 
...
dotplot = df %>% 
  ...
  mutate(condition = factor(condition, levels = col_clustered$labels[col_clustered$order]),
         genes = factor(genes, levels = row_clustered$labels[row_clustered$order])) %>% 
  ggplot(aes(x=condition, y = genes, color = log2fold_change, size = padj)) +
  geom_point() + 
  theme_few() + 
