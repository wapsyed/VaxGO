...
### 13 Vax & COVID-19
p_adj < 0.05, no Log2-fold change filter.
```{r}
# Reactive expression to perform ORA analyses
ora_results_13Vax_filtered <- reactive({
  req(input$go_ora)
  req(df_ora())
  
  # Perform ORA for ImmuneGO general
  degs_ORA_VAX_filtered <- df_ora() %>%
    autoORA(TERM2GENE = covid_13vax_Genes_filtered,
            geneset_name = "COVID-19 and 13 Vax Atlas, filtered",
            pAdjustMethod = input$pAdjustMethod,
            pvalueCutoff_ora = input$pvalueCutoff_ora,
            qvalueCutoff_ora = input$qvalueCutoff_ora)
})
...
renderDT({
  req(input$go_ora)
  req(ora_results_13Vax_filtered())
  ora_results_13Vax_filtered() %>% 
    datatable(extensions = 'Buttons',
...
renderPlotly({
  ...
  # Prepare the data
  data <- ora_results_13Vax_filtered()%>% 
   group_by(condition) %>%
    arrange(condition, desc(-log(qvalue))) %>% 
    slice_head(n = 5) %>%
    ungroup() %>%
...
```{r}
# Reactive: Filtered data for correlation
df_corr <- reactive({
  req(input$go_corr)
  req(conditions_genes())
...
# Reactive: Compute correlation matrix
corr_results <- reactive({
  ...
  cor_matrix <- cor(df_wide, method = tolower(input$corr_metric))
...
# Render correlation matrix in DataTable
DT::renderDataTable({
  req(input$go_corr)
  req(corr_results())
...
input %>% 
    filter(log2fold_change >= 1 | 
             log2fold_change <= - 1,
           padj < 0.05) %>% 
   select(process = condition, genes, value = log2fold_change) %>%
   bind_rows(degs_covid_13vax_hagan_syed %>% 
                  select(process = condition, genes, value = log2fold_change)) %>% 
   distinct() %>% 
...
# Reactive: Compute correlation matrix
corr_results_13vax_covid <- reactive({
  req(input$go_corr)
  req(df_corr())
  ...
  df_wide <- df_corr() %>%
    bind_rows(degs_covid_13vax_hagan_syed %>% 
                 select(process = condition, genes, value = log2fold_change))
    pivot_wider(names_from = genes, values_from = value, values_fill = NA, values_fn = mean) %>%
    column_to_rownames("process")
...
DT::renderDataTable({
  req(input$go_corr)
  req(corr_results_13vax_covid())
  
  corr_results_13vax_covid() %>%
    datatable(extensions = 'Buttons',
              options = list(
                paging = TRUE,
                searching = TRUE,
