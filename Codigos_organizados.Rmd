...
ora_results_final
```
#### VAXGO ORA (CONTINUAR COM ENRICHER)
```{r}
# Filter non-neutral DEGs
df <- degs %>% 
  filter(direction != "NEUTRAL")
...
Fazer o mesmo para todos os gene sets
```{r}
##### INPUT
...
degs_GSEA_ImmuneGO_General <- degs %>%
  ...
  autoGSEA(TERM2GENE = ImmuneGO_genes_general, 
           ...
           pAdjustMethod = pAdjustMethod) %>% 
  rownames_to_column("delete") %>% 
  select(!c("delete", "Description")) %>% 
  select(condition, process = ID, everything()) %>% 
  clean_names() %>% 
...
write_csv(degs_GSEA_ready, 
          file = here("Example", paste0(filename, "_GSEA_ALL_VAXGO_", today(), ".csv")))
