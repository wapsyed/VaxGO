...
dist_jaccard <- vegdist(matrix, binary = T, method = "jaccard", diag = T, upper = T)

jaccard.matrix = as.matrix(dist_jaccard)
jaccard.matrix = 1-jaccard.matrix
...
#Salvar resultados
write.csv(shared_genes_df_mirror, file = here("Tables", "CellMarker_Immune_sharedgenes_Fishers_Exact_Test.csv")) #Alterar
...
# Values colors
col_fun <- colorRamp2(c(0, 0.5), c("white", "#ed6a5a"))
...
plot_hist_immunego = ggplot(ImmuneGO_Annotated_GO) +
  ...
  geom_text(aes(x = setsize_intervals,  y =count, label = set_size)) +
  labs(title = "Gene sets distribution by size",
       fill = "Immune System",
       x = "Set size",
       y = "Frequency") +
  theme_vaxgo() +
...
heatmap_plot = Heatmap(
  ...
  cell_fun = function(j, i, x, y, width, height, fill) {
    grid.text(sprintf("%.0f", matrix[i, j]), x, y, gp = gpar(fontsize = 6))
  },
  show_row_names = TRUE,
  row_names_side = "left",
  show_heatmap_legend = TRUE,
...
row_ha = HeatmapAnnotation(df = ann_cols_heatmap,
                       col = ann_vaxsig_colors,
                       which= "row",
                       show_annotation_name = F,
                       show_legend = F)
...
# 2.Overlapping with other vaccines
